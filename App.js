...
export default class App extends Component<Props> {
  ...
  async componentDidMount() {
    branch.subscribe(({ error, params }) => {
      ...
      let openChoice = event.content.teamHome === params.choice
          ? event.content.teamAway
          : event.content.teamHome;
      console.log('openChoice', openChoice);
      let message = 'Would you like to bet on ' + openChoice + ' for the event ' + event.title + ' for ' + params.amount + ' ether?';
  ...
  renderContent(event) {
    ...
    return (
        <View style={[styles.content, {flexDirection:'column', padding:10, borderBottomWidth:1, borderBottomColor:'#eeeeee'}]}>
